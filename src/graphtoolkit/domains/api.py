...
class APIValidatorInput(BaseModel):
    """Input schema for API validator phase."""
    implementation: Dict[str, str] = Field(description='Generated API code')
    tests: Dict[str, str] = Field(description='Generated test files')
    openapi_spec: Dict[str, Any] = Field(description='OpenAPI specification')


class APIValidatorOutput(BaseModel):
    """Output schema for API validator phase."""
...
API_VALIDATOR_PHASE = PhaseDefinition(
    ...
    input_schema=APIValidatorInput,
    output_schema=APIValidatorOutput,
    dependencies=['generator'],
    templates=TemplateConfig(
        system_template='system/api/validator',
        user_template='prompts/api/validate_implementation',
        variables={
            'framework': 'Testing',
            'purpose': 'Validate API implementation'
        }
    ),
    storage_pattern='workflow/{workflow_id}/api_validator',
    storage_type=StorageType.KV,
...
def register_api_domain():
    ...
    registry.register_phase('api.validator', API_VALIDATOR_PHASE)
